...
function askLetter() {
    ...
    rl.question(`Lettre (ou " " pour valider le mot, "stop" pour quitter) : `, function(input) {
        if (input === 'stop') {
            console.log(`\n Phrase finale : ${R.join(' ', [...phrase, currentWord])}`);
            rl.close();
            return;
        }

        if (input === ' ') {
            if (R.length(currentWord) > 0) {
                phrase.push(currentWord);
                currentWord = '';
                console.log(` Phrase : ${R.join(' ',phrase)}\n`);
            }
            ...
            if (options) {
                ...
                const formatPredictions = R.pipe(
                    R.map(([w, p]) => `${w} (${p.toFixed(2)})`),
                    R.join(', ')
                );

                console.log(` Prochain(s) mot(s) possible(s) : ${formatPredictions(predictions)}`);
            } else {
        ...
        if (options) {
            ...
            if (R.pipe(R.keys, R.length)(nextLetterProbs) > 0) {
                console.log(` Probabilités des prochaines lettres :`);
                const sorted = R.pipe(
                    R.toPairs,                                         // comme Object.entries
                    R.sort(R.descend(R.nth(1))),                       // tri par valeur décroissante
                    R.map(([l, p]) => ({
                        lettre: l === ' ' ? '(espace)' : l,
